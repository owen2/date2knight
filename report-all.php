...
function printHearts($score, $char)
{
    ...
    function report($personA)
    {
    ?>
    <div class="padded bodywrap content pagebreak">
    <h1><?php echo($personA['firstname'] . " " . $personA['lastname'] . "<br>Mailbox " . $personA['box']);?></h1>
    <?php getTopDates($personA['id'], 10); ?>
    <br><br>
...
<body>
    <?php
       require_once("connect.php");
	    $result = mysql_query("SELECT * FROM `response` INNER JOIN profile ON(profile.id=response.profile_id) WHERE profile.paid=1 GROUP BY profile.id");
